...
export const WhatsAppFloat: React.FC = () => {
  ...
  // Check if we're on a subdomain (store) using SubdomainService
  useEffect(() => {
    const checkSubdomain = async () => {
      try {
        const info = await SubdomainService.detectSubdomain();
        setIsSubdomain(info.isSubdomain);
      } catch (error) {
  ...
  // Don't show on store subdomains - stores have their own WhatsApp button
  if (isSubdomain) {
    return null;
  }
  ...
  return (
    ...
    >
      <AnimatePresence>
        {showTooltip && (
          <motion.div
            ...
            exit={{ opacity: 0, x: 20 }}
            className="absolute right-full mr-3 top-1/2 transform -translate-y-1/2 bg-gray-900 text-white px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap shadow-lg"
          >
      ...
      >
        <motion.div
          className="flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-green-500 to-green-600 rounded-full shadow-2xl hover:shadow-green-500/50 transition-all duration-300 cursor-pointer"
