  ...
  return (
    <StoreContext.Provider value={{
      ...
      <div className="min-h-screen bg-gray-50 storefront-body" style={{ backgroundColor: business?.branding?.storeBackgroundColor || '#1c1c1e' }}>
        ...
        {business && getBusinessWhatsAppConfig(business) && (
          <div className="fixed bottom-6 right-6 z-50">
            <a
              href={getBusinessWhatsAppConfig(business)?.whatsappUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="group flex items-center justify-center w-12 h-12 bg-green-500 hover:bg-green-600 text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110"
              title={`Chat with us on WhatsApp: ${getBusinessWhatsAppConfig(business)?.formattedNumber}`}
            >
            ...
            <div className="absolute bottom-full right-0 mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap pointer-events-none">
              {getBusinessWhatsAppConfig(business)?.formattedNumber}
