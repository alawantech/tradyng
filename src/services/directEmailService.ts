...
class DirectEmailService {
  ...
  static async sendPasswordResetOTP(email: string, otp: string, storeName: string, whatsappNumber?: string): Promise<boolean> {
    try {
      this.initialize();
      ...
      const msg = {
        to: email,
        from: 'noreply@rady.ng',
        subject: `${otp} - ${storeName} password reset code`,
        html: `
          <div style="max-width: 500px; margin: 0 auto; font-family: Arial, sans-serif; background: white; border-radius: 10px; padding: 30px; text-align: center;">
            ...
            <!-- OTP Code - Large and Prominent -->
            <div style="background: #3B82F6; color: white; border-radius: 8px; padding: 20px; margin: 20px 0;">
              <p style="margin: 0 0 10px; font-size: 14px;">Your password reset code:</p>
              <div style="font-size: 36px; font-weight: bold; letter-spacing: 4px; font-family: monospace;">
                ${otp}
              </div>
            </div>
            ...
            <!-- Simple Instructions -->
            <p style="margin: 20px 0 0; color: #666; font-size: 14px;">
              Enter this code to reset your password. Expires in 5 minutes.
            </p>
      ...
      await sgMail.send(msg);
      console.log('✅ Password reset email sent successfully via direct SendGrid');
      return true;
    } catch (error: any) {
      console.error('❌ Direct SendGrid password reset error:', error);
      return false;
    }
  }
}
