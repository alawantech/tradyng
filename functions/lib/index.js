...
exports.sendMessageNotification = functions.https.onRequest({
    ...
    try {
        ...
        const { businessId, customerId, message, sender, senderName, customerEmail, customerName, businessName, businessEmail } = req.body || {};
...
exports.sendPaymentReceiptNotification = functions.https.onRequest({
    secrets: ['MAIL_SENDER_API_TOKEN']
}, async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
    ...
    try {
        ...
        const { customerEmail, customerName, orderId, businessName, businessEmail, businessPhone } = req.body || {};
...
exports.sendAdminPaymentReceiptNotification = functions.https.onRequest({
    secrets: ['MAIL_SENDER_API_TOKEN']
}, async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
    ...
    try {
        ...
        const { adminEmail, customerName, customerEmail, orderId, businessName, businessId } = req.body || {};
...
exports.sendOrderDeliveryEmail = functions.https.onRequest({
    secrets: ['MAIL_SENDER_API_TOKEN']
}, async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
    ...
    try {
        ...
        const { customerEmail, customerName, customerPhone, shippingAddress, orderId, businessName, businessPhone, items, total, paymentMethod, createdAt, currency, pdfBase64 } = req.body || {};
...
exports.sendOrderApprovalEmail = functions.https.onRequest({
    secrets: ['MAIL_SENDER_API_TOKEN']
}, async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
...
async function sendEmailWithAttachment(to, subject, html, attachmentBuffer, attachmentName) {
    var _a, _b;
    ...
    try {
        ...
        if (sendgridKey) {
            try {
                return await trySendGridWithAttachment();
            }
            catch (sgErrUnknown) {
                const sgMsg = extractMsg(sgErrUnknown);
...
exports.initializePayment = functions.https.onRequest({
    secrets: []
}, async (req, res) => {
    var _a;
    res.set('Access-Control-Allow-Origin', '*');
    ...
    try {
        ...
        const { amount, currency, customerEmail, customerName, customerPhone, txRef, redirectUrl, meta } = req.body || {};
        ...
        console.log('✅ Payment initialized successfully:', (_a = data.data) === null || _a === void 0 ? void 0 : _a.link);
        res.json({
            status: 'success',
...
exports.verifyPaymentAndAwardCommission = functions.https.onRequest({
    secrets: []
}, async (req, res) => {
    var _a, _b, _c;
    res.set('Access-Control-Allow-Origin', '*');
    ...
    try {
        ...
        const verifyResponse = await (0, node_fetch_1.default)(`https://api.flutterwave.com/v3/transactions/${encodeURIComponent(txRef)}/verify`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${flutterwaveSecretKey}`,
                'Content-Type': 'application/json'
        ...
        console.log('✅ Payment verified:', (_a = verifyData.data) === null || _a === void 0 ? void 0 : _a.status);
        // Check if payment was successful
        if (((_b = verifyData.data) === null || _b === void 0 ? void 0 : _b.status) === 'successful') {
            const meta = ((_c = verifyData.data) === null || _c === void 0 ? void 0 : _c.meta) || {};
...
async function awardAffiliateCommission(params) {
    ...
    const affiliate = Object.assign({ id: affiliateDoc.id }, affiliateDoc.data());
    ...
    if (businessId) {
        try {
            ...
            if (businessDoc.exists) {
                businessName = ((_a = businessDoc.data()) === null || _a === void 0 ? void 0 : _a.name) || businessName;
            }
        }
        catch (error) {
